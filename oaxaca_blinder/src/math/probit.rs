...
pub fn probit(y: &DVector<f64>, x: &DMatrix<f64>, max_iter: usize, tol: f64) -> Result<ProbitResult, OaxacaError> {
    let n = x.nrows();
    let k = x.ncols();
    ...
    for iter in 0..max_iter {
        ...
        let h_inv = match h.clone().try_inverse() {
            Some(inv) => inv,
            None => return Err(OaxacaError::NalgebraError("Failed to invert Hessian in Probit".to_string())),
        };
    ...
    let vcov = match h.try_inverse() {
        Some(inv) => -inv,
        None => return Err(OaxacaError::NalgebraError("Failed to invert Hessian for VCOV".to_string())),
    };
