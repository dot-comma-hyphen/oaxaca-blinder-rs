...
impl From<PolarsError> for AkmError {
    fn from(err: PolarsError) -> Self { AkmError::PolarsError(err) }
}
...
impl std::fmt::Display for AkmError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            AkmError::ConvergenceFailed(s) => write!(f, "Convergence failed: {}. Try increasing max_iters or checking for collinearity.", s),
        }
    }
}
...
impl AkmBuilder {
    ...
    pub fn run(self) -> Result<AkmResult, AkmError> {
        ...
        let df_connected = find_largest_connected_set(&self.dataframe, &self.worker_col, &self.firm_col)?;
        
        if df_connected.height() == 0 {
            return Err(AkmError::NotEnoughData("No connected set found".to_string()));
        }

        // 2. Solve AKM
        solve_akm(df_connected, &self.outcome, &self.worker_col, &self.firm_col, &self.controls, self.tolerance, self.max_iters)
    }
}
...
fn find_largest_connected_set(df: &DataFrame, worker_col: &str, firm_col: &str) -> Result<DataFrame, AkmError> {
    ...
    let largest_component_root = component_sizes.iter()
    ...
    let mask: BooleanChunked = workers.str()?.into_iter().zip(firms.str()?.into_iter())
        .map(|(w_opt, f_opt)| {
            if let (Some(w), Some(f)) = (w_opt, f_opt) {
                if let (Some(&w_idx), Some(&f_idx)) = (worker_map.get(w), firm_map.get(f)) {
...
fn solve_akm(
    ...
    let worker_indices: Vec<u32> = workers.str()?.into_iter()
        .map(|opt| worker_map.get(opt.unwrap()).unwrap().clone())
        .collect();
        
    let firm_indices: Vec<u32> = firms.str()?.into_iter()
    ...
    demean_vector(&mut y_resid, &worker_indices, &firm_indices, n_workers, n_firms, tolerance, max_iters)?;
    
    let mut x_resids = x_vectors.clone();
    for x_vec in &mut x_resids {
        demean_vector(x_vec, &worker_indices, &firm_indices, n_workers, n_firms, tolerance, max_iters)?;
    }
    ...
    let beta = if n_controls > 0 {
        ...
        let chol = nalgebra::linalg::Cholesky::new(xtx).ok_or(AkmError::ConvergenceFailed("OLS design matrix is singular".to_string()))?;
        chol.solve(&xty)
    } else {
        DVector::zeros(0)
    };
    ...
    let (alpha, psi) = recover_fe(&r, &worker_indices, &firm_indices, n_workers, n_firms, tolerance, max_iters)?;
    ...
    // 6. Format Output
    let worker_ids: Vec<String> = unique_workers.str()?.into_iter().flatten().map(|s| s.to_string()).collect();
    let firm_ids: Vec<String> = unique_firms.str()?.into_iter().flatten().map(|s| s.to_string()).collect();
...
fn demean_vector(
    ...
    for &idx in worker_indices { worker_counts[idx as usize] += 1; }
    for &idx in firm_indices { firm_counts[idx as usize] += 1; }
...
fn recover_fe(
    ...
    while diff > tolerance && iter < max_iters {
        ...
        // Check convergence
        diff = 0.0;
        for i in 0..n_workers { diff += (alpha[i] - prev_alpha[i]).powi(2); }
        for j in 0..n_firms { diff += (psi[j] - prev_psi[j]).powi(2); }
        diff = diff.sqrt();
