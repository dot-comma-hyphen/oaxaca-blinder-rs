...
impl WageScaleProblem {
    pub fn new(census: DataFrame, budget: f64, grades: u32, steps: u32) -> Self {
        Self {
            census,
            budget,
            min_step_diff: 0.03,
    ...
    fn define_constraints<T: SolverModel>(&self, variables: &VariableMap, problem: &mut T) -> Result<()> {
        ...
        for i in 0..df.height() {
             ...
             if let Some(max_sal) = m_ca.get(i) {
                 if let Some(var) = variables.get(&format!("step_{}_{}", g, s)) {
                     // Use Expression to express the constraint since Variable method was causing issues
                     // Expression::from(*var).geq(*max_sal)
